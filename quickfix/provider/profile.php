...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update_profile') {
        ...
        if (empty($full_name) || empty($email) || empty($username)) {
            ...
            $email_check = $db->prepare("SELECT id FROM users WHERE email = :email AND id != :provider_id");
            $email_check->execute([':email' => $email, ':provider_id' => $provider_id]);
            
            // Check if username is already taken by another user
            $username_check = $db->prepare("SELECT id FROM users WHERE username = :username AND id != :provider_id");
            $username_check->execute([':username' => $username, ':provider_id' => $provider_id]);
        ...
        if (empty($current_password) || empty($new_password) || empty($confirm_password)) {
            ...
            $password_update = $db->prepare("UPDATE users SET password = :password, updated_at = CURRENT_TIMESTAMP WHERE id = :provider_id");
            
            if ($password_update->execute([':password' => $hashed_password, ':provider_id' => $provider_id])) {
        ...
        if (isset($_FILES['profile_photo']) && $_FILES['profile_photo']['error'] === 0) {
            ...
            if (!in_array($file['type'], $allowed_types)) {
                ...
                if (move_uploaded_file($file['tmp_name'], $filepath)) {
                    ...
                    // Update database
                    $photo_update = $db->prepare("UPDATE users SET profile_image = :filename, updated_at = CURRENT_TIMESTAMP WHERE id = :provider_id");
                    
                    if ($photo_update->execute([':filename' => $filename, ':provider_id' => $provider_id])) {
...
$stats_query = "SELECT 
    COUNT(DISTINCT CASE WHEN ps.service_id IS NOT NULL THEN ps.service_id END) as total_services,
    COUNT(DISTINCT CASE WHEN b.id IS NOT NULL THEN b.id END) as total_bookings,
...
$stats_stmt->execute([':provider_id' => $provider_id]);
...
<head>
    ...
    <title>Provider Profile Settings - <?php echo SITE_NAME; ?></title>
    ...
    <nav class="navbar">
        <div class="nav-container">
            <a href="dashboard.php" class="logo">
                <i class="fas fa-tools"></i> QuickFix Pro
            </a>
            <ul class="nav-menu">
        ...
        <div class="dashboard-container">
            <div class="dashboard-header">
                <h1 class="dashboard-title">
                    <i class="fas fa-user-edit"></i> Provider Profile
                </h1>
                <p class="dashboard-subtitle">
                    Manage your service provider account settings and preferences
                </p>
                    ...
                    <div class="profile-header">
                        ...
                        <div class="profile-info">
                            ...
                            <div class="profile-status">
                                <span class="status-badge provider">
                                    <i class="fas fa-tools"></i>
                                    Service Provider
                                </span>
                                <span class="join-date">
                                    <i class="fas fa-calendar"></i>
                                    Provider since <?php echo date('F Y', strtotime($user['created_at'])); ?>
                                </span>
                            </div>
                        </div>
                                ...
                                <div class="enhanced-form-group">
                                    ...
                                    <div class="input-container">
                                        <input type="text" 
                                               ...
                                               required>
                                        <small class="form-help">Choose a unique username for your provider account</small>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="enhanced-form-group">
                                ...
                                <div class="textarea-container">
                                    <textarea name="address" 
                                              ...
                                              placeholder="Enter your service area or business address..."><?php echo htmlspecialchars($user['address']); ?></textarea>
                                </div>
                            </div>
                        </div>
